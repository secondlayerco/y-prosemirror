  ...
  if (
    ...
    if (relSel.type === "all") {
      ...
      if (anchor !== null && head !== null) {
        console.debug("[y-prosemirror] restoreRelativeSelection", {
          anchor,
          head,
          relSel,
        });
...
const equalYTextPText = (ytext, ptexts) => {
  ...
  return (
    ...
    delta.every(
      /** @type {(d:any,i:number) => boolean} */ (d, i) =>
        ...
        object.every(d.attributes, (attr, yattrname) => {
          ...
          return equalAttrs(
            attr,
            pmarks.find(
              /** @param {any} mark */ (mark) => mark.type.name === markname,
            )?.attrs,
          );
        }),
    )
...
export const attributesToMarks = (attrs, schema) => {
  ...
  console.log("[ERWAN] attributesToMarks", marks);
  return marks;
};
